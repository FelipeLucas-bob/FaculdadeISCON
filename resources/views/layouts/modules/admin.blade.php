@php
    // Filtra apenas itens do tipo 'admin' que o usuário pode ver
    $adminItems = collect($sidebarItems)->filter(function ($side) {
        if (!isset($side['type']) || $side['type'] !== 'admin') {
            return false;
        }
    ...
    <li class="menu {{ $isActive ? 'active' : '' }}">
        @if ($hasSubitems)
            ...
            <ul class="collapse submenu list-unstyled {{ $isActive ? 'show' : '' }}"
                ...
                @foreach ($side['subitems'] as $submenu)
                    @if (!empty($submenu['admin']) && !Auth::user()->isSuperAdmin() && $side['type'] !== 'admin')
                        @continue
                    @endif
