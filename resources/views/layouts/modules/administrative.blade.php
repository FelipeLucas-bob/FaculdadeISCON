@php
    // Filtra apenas itens do tipo 'administrative' que o usuário pode ver
    $administrativeItems = collect($sidebarItems)->filter(function ($side) {
        if (!isset($side['type']) || $side['type'] !== 'administrative') {
            return false;
        }
...
@if ($administrativeItems->count() > 0)
...
@foreach ($administrativeItems as $side)
    @php
    ...
    <li class="menu {{ $isActive ? 'active' : '' }}">
        @if ($hasSubitems)
            ...
            <ul class="collapse submenu list-unstyled {{ $isActive ? 'show' : '' }}"
                ...
                @foreach ($side['subitems'] as $submenu)
                    @if (!empty($submenu['admin']) && !Auth::user()->isSuperAdmin() && $side['type'] !== 'administrative')
                        @continue
                    @endif
